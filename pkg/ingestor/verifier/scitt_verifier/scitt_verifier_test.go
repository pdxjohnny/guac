...
import (
	...
	"github.com/scitt/scitt/pkg/signature/dsse"
)
...
func TestSCITTVerifier_Verify(t *testing.T) {
	ctx := logging.WithLogger(context.Background())
	setupOneProvider(t)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sigVerifier := NewSCITTAndKeyVerifier()
			got, err := sigVerifier.Verify(ctx, tt.doc.Blob)
			if (err != nil) != tt.wantErr {
				t.Errorf("SCITTVerifier.Verify() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if err == nil {
				if !reflect.DeepEqual(got, tt.want) {
					t.Errorf("SCITTVerifier.Verify() = %v, want %v", got, tt.want)
				}
			}
			if sigVerifier.Type() != "scitt" {
				t.Errorf("sigVerifier.Type() = %s, want %s", sigVerifier.Type(), "scitt")
			}
		})
	}
}

func TestMultiSignatureSCITTVerifier_Verify(t *testing.T) {
	ctx := logging.WithLogger(context.Background())
	setupOneProvider(t)
	foundECDSAKey, err := key.Find(ctx, ecdsaKeyID)
	if err != nil {
